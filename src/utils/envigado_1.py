...
def obtener_casas():
    ...
    base_url = "https://www.fincaraiz.com.co/arriendo/apartamentos/envigado/antioquia"
    ...
    while current_page <= max_pages:
        ...
        for prop in propiedades:
            try:
                ...
                # Extraer título/ubicación - En Fincaraíz el formato es "Apartamento en arriendo en [barrio], envigado"
                titulo = ""
                
                # Buscar el patrón típico de Fincaraíz: "Apartamento en arriendo en [barrio], envigado"
                titulo_match = re.search(r'(Apartamento|Casa|Local|Oficina|Apartaestudio|Lote).*?en arriendo.*?en\s+([^,]+),\s*envigado', texto_completo, re.I)
                if titulo_match:
                    tipo = titulo_match.group(1)
                    barrio = titulo_match.group(2).strip()
                    titulo = f"{tipo} en arriendo, Envigado, {barrio}"
                else:
                    # Buscar formato alternativo: "Apartamento en Envigado, Antioquia"
                    titulo_match = re.search(r'(Apartamento|Casa|Local|Oficina|Apartaestudio|Lote)\s+en\s+Envigado[^,]*,\s*Antioquia', texto_completo, re.I)
                    if titulo_match:
                        tipo = titulo_match.group(1)
                        # Intentar extraer barrio del texto completo
                        barrio_match = re.search(r'Envigado[^,]*,\s*Antioquia[^,]*?([^,]+)', texto_completo, re.I)
                        if barrio_match:
                            barrio = barrio_match.group(1).strip()
                            titulo = f"{tipo} en arriendo, Envigado, {barrio}"
                        else:
                            titulo = f"{tipo} en arriendo, Envigado"
                    else:
                ...
                # Extraer precio - Fincaraíz usa formato "$ 5.900.000" o "$ 4.950.000"
                precio = 0
                ...
                # Formato típico: "2 Habs." "1 Baño" "84 m²"
                
                # Buscar habitaciones - formato "2 Habs." o "1 Hab"
                habitaciones = 0
                ...
                # Buscar baños - formato "1 Baño" o "2 Baños"
                banios_count = 0
                ...
                # Buscar metros cuadrados - formato "84 m²" o "90 m²"
                metros = 0
                metros_match = re.search(r'(\d+)\s*m[²2]', texto_completo, re.I)
                if metros_match:
                    metros = int(metros_match.group(1))
                else:
                    # Buscar formato alternativo "84 mt2" o "90 mt2"
