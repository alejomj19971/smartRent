...
def obtener_casas():
    ...
    base_url = "https://www.fincaraiz.com.co/arriendo/apartamentos/caldas/antioquia"
    url = base_url
    ...
    while current_page <= max_pages:
        ...
        for prop in propiedades:
            try:
                ...
                # Extraer título/ubicación - En Fincaraíz el formato es "Apartamento en arriendo en [barrio], caldas"
                titulo = ""
                
                # Buscar el patrón típico de Fincaraíz: "Apartamento en arriendo en [barrio], caldas"
                titulo_match = re.search(r'(Apartamento|Casa|Local|Oficina|Apartaestudio|Lote).*?en arriendo.*?en\s+([^,]+),\s*caldas', texto_completo, re.I)
                if titulo_match:
                    tipo = titulo_match.group(1)
                    barrio = titulo_match.group(2).strip()
                    titulo = f"{tipo} en arriendo, Caldas, {barrio}"
                else:
                    # Buscar formato alternativo: "Apartamento en Caldas, Antioquia"
                    titulo_match = re.search(r'(Apartamento|Casa|Local|Oficina|Apartaestudio|Lote)\s+en\s+Caldas[^,]*,\s*Antioquia', texto_completo, re.I)
                    if titulo_match:
                        tipo = titulo_match.group(1)
                        # Intentar extraer barrio del texto completo
                        barrio_match = re.search(r'Caldas[^,]*,\s*Antioquia[^,]*?([^,]+)', texto_completo, re.I)
                        if barrio_match:
                            barrio = barrio_match.group(1).strip()
                            titulo = f"{tipo} en arriendo, Caldas, {barrio}"
                        else:
                            titulo = f"{tipo} en arriendo, Caldas"
                    else:
                ...
                # Extraer precio - Fincaraíz usa formato "$ 1.500.000" o "$ 850.000"
                precio = 0
                ...
                # Formato típico: "2 Habs." "2 Baños" "49 m²"
                
                # Buscar habitaciones - formato "2 Habs." o "3 Hab"
                habitaciones = 0
                ...
                # Buscar baños - formato "2 Baños" o "1 Baño"
                banios_count = 0
                ...
                # Buscar metros cuadrados - formato "49 m²" o "55 m²"
                metros = 0
                metros_match = re.search(r'(\d+)\s*m[²2]', texto_completo, re.I)
                if metros_match:
                    metros = int(metros_match.group(1))
                else:
                    # Buscar formato alternativo "49 mt2" o "55 mt2"
