...
def obtener_casas():
    ...
    base_url = "https://www.fincaraiz.com.co/arriendo/apartamentos/bello/antioquia"
    ...
    while current_page <= max_pages:
        ...
        for prop in propiedades:
            try:
                ...
                # Extraer título/ubicación - En Fincaraíz el formato es "Apartamento en arriendo en [barrio], bello"
                titulo = ""
                
                # Buscar el patrón típico de Fincaraíz: "Apartamento en arriendo en [barrio], bello"
                titulo_match = re.search(r'(Apartamento|Casa|Local|Oficina|Apartaestudio|Lote).*?en arriendo.*?en\s+([^,]+),\s*bello', texto_completo, re.I)
                if titulo_match:
                    tipo = titulo_match.group(1)
                    barrio = titulo_match.group(2).strip()
                    titulo = f"{tipo} en arriendo, Bello, {barrio}"
                else:
                    # Buscar formato alternativo: "Apartamento en Bello, Antioquia"
                    titulo_match = re.search(r'(Apartamento|Casa|Local|Oficina|Apartaestudio|Lote)\s+en\s+Bello[^,]*,\s*Antioquia', texto_completo, re.I)
                    if titulo_match:
                        tipo = titulo_match.group(1)
                        # Intentar extraer barrio del texto completo
                        barrio_match = re.search(r'Bello[^,]*,\s*Antioquia[^,]*?([^,]+)', texto_completo, re.I)
                        if barrio_match:
                            barrio = barrio_match.group(1).strip()
                            titulo = f"{tipo} en arriendo, Bello, {barrio}"
                        else:
                            titulo = f"{tipo} en arriendo, Bello"
                    else:
                ...
                # Extraer precio - Fincaraíz usa formato "$ 2.200.000" o "$ 1.400.000"
                precio = 0
                ...
                # Formato típico: "3 Habs." "2 Baños" "63 m²"
                
                # Buscar habitaciones - formato "3 Habs." o "2 Hab"
                habitaciones = 0
                ...
                # Buscar metros cuadrados - formato "63 m²" o "65 m²"
                metros = 0
                metros_match = re.search(r'(\d+)\s*m[²2]', texto_completo, re.I)
                if metros_match:
                    metros = int(metros_match.group(1))
                else:
                    # Buscar formato alternativo "63 mt2" o "65 mt2"
