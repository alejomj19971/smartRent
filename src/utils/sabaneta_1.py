...
def obtener_casas():
    ...
    base_url = "https://www.fincaraiz.com.co/arriendo/sabaneta/antioquia"
    url = base_url
    ...
    while current_page <= max_pages:
        ...
        for prop in propiedades:
            try:
                ...
                # Extraer título/ubicación - En Fincaraíz el formato es "Apartamento en arriendo en [barrio], sabaneta"
                titulo = ""
                
                # Buscar el patrón típico de Fincaraíz: "Apartamento en arriendo en [barrio], sabaneta"
                titulo_match = re.search(r'(Apartamento|Casa|Local|Oficina|Apartaestudio|Lote).*?en arriendo.*?en\s+([^,]+),\s*sabaneta', texto_completo, re.I)
                if titulo_match:
                    tipo = titulo_match.group(1)
                    barrio = titulo_match.group(2).strip()
                    titulo = f"{tipo} en arriendo, Sabaneta, {barrio}"
                else:
                    # Buscar formato alternativo: "Apartamento en Sabaneta, Antioquia"
                    titulo_match = re.search(r'(Apartamento|Casa|Local|Oficina|Apartaestudio|Lote)\s+en\s+Sabaneta[^,]*,\s*Antioquia', texto_completo, re.I)
                    if titulo_match:
                        tipo = titulo_match.group(1)
                        # Intentar extraer barrio del texto completo
                        barrio_match = re.search(r'Sabaneta[^,]*,\s*Antioquia[^,]*?([^,]+)', texto_completo, re.I)
                        if barrio_match:
                            barrio = barrio_match.group(1).strip()
                            titulo = f"{tipo} en arriendo, Sabaneta, {barrio}"
                        else:
                            titulo = f"{tipo} en arriendo, Sabaneta"
                    else:
                ...
                # Extraer precio - Fincaraíz usa formato "$ 1.950.000" o "$ 3.800.000"
                precio = 0
                ...
                # Formato típico: "3 Habs." "2 Baños" "98 m²"
                
                # Buscar habitaciones - formato "3 Habs." o "1 Hab"
                habitaciones = 0
                ...
                # Buscar metros cuadrados - formato "98 m²" o "52 m²"
                metros = 0
                metros_match = re.search(r'(\d+)\s*m[²2]', texto_completo, re.I)
                if metros_match:
                    metros = int(metros_match.group(1))
                else:
                    # Buscar formato alternativo "98 mt2" o "52 mt2"
